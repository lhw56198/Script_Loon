...
@Adapted for Loon by Perplexity

[rewrite]
^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus script-request-header WeTalk.loon.js

[script]
cron "20 8,20 * * *" script-path=WeTalk.loon.js, tag=WeTalk签到
...
// ====== Loon 持久化（$persistentStore 替代 $prefs） ======
function loadStore() {
...
// ====== 核心逻辑（网络请求：$task.fetch → $httpClient.get） ======
...
function runAccount(acc, index, total) {
  ...
  function doVideoLoop(count) {
    ...
    function next() {
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          i++;
          fetchApi('videoBonus', acc.capture, headers).then(res => {
            try {
  ...
  return fetchApi('queryBalanceAndBonus', acc.capture, headers).then(res => {
    try {
      const d = JSON.parse(res.body);
      if (d.retcode === 0) msgs.push(`💰 余额：${d.result.balance} Coins`);
      else msgs.push(`⚠️ 查询：${d.retmsg}`);
    } catch (e) { msgs.push('❌ 查询：解析失败'); }
    return fetchApi('checkIn', acc.capture, headers);
  }).then(res => {
    try {
      const d = JSON.parse(res.body);
  ...
  }).then(() => fetchApi('queryBalanceAndBonus', acc.capture, headers)).then(res => {
    try {
      const d = JSON.parse(res.body);
