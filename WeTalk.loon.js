// WeTalk for Loon
// 基于原 WeTalk.js 改写为 Loon 独立脚本版
// 原逻辑：抓 queryBalanceAndBonus 请求保存账号参数；定时执行 queryBalanceAndBonus / checkIn / videoBonus [file:1]
...
function fetchApiWithCapture(path, capture, headers) {
  return $task.fetch({
...
function runAccount(acc, index, total) {
  ...
  const ua = buildUA(acc.baseUA, acc.uaSeed || index);
  const headers = buildHeaders(acc.capture, ua);
  const msgs = [tag];

  function fetchApi(path) {
    return fetchApiWithCapture(path, acc.capture, headers);
  }
        ...
        setTimeout(() => {
          ...
          fetchApi('videoBonus').then(res => {
            try {
              ...
              if (d.retcode === 0) {
                msgs.push(`🎬 视频${i}：+${d.result && d.result.bonus != null ? d.result.bonus : '?'} Coins`);
                resolve(next());
              } else {
  ...
  return fetchApi('queryBalanceAndBonus')
    .then(res => {
      try {
        ...
        else msgs.push(`⚠️ 查询：${d.retmsg || '失败'}`);
      } catch (e) {
        msgs.push('❌ 查询：解析失败');
      }
    ...
    .then(res => {
      try {
        ...
        else msgs.push(`⚠️ 签到：${d.retmsg || '失败'}`);
      } catch (e) {
        msgs.push('❌ 签到：解析失败');
      }
...
if (typeof $request !== 'undefined' && $request) {
  ...
  if (!email) {
    ...
    const accId = email || fingerprintOf(paramsRaw);
    const now = Date.now();
